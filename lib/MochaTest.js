...
class MochaTest {
    ...
    async runTest(){
        ...
        writeTempFile(tempFilePath, studentFiles);
        const mochaTestFile = await getTestFilePath(this.testFile);
...
async function getTestFilePath(fileName) {
    return await getFilePath(mochaTestsPath, fileName);
}
...
function writeTempFile(path, string){
